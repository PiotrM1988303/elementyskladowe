...
let moneyArr = [0];
// money przechowuje wszystkie przychody i wydatki
...
const createNewTransaction = () => {
    ...
    countMoney(moneyArr)
    // wywołuje funkcje, której argumentem jest tablica globalna przechowująca pieniądze
    closePanel();
    ID++;
    clearInputs();
}
...
// jak parametr count money należy patrzeć jako tablice arrayMoney
// Reduce() zwraca pojedynczą wartość, która jest skumulowanym wynikiem funkcji.
const countMoney = money => {
    const newMoney = money.reduce((a, b) => a + b);
    availableMoney.textContent = `${newMoney}zł`;
    // w divie available many mazostać przypisana wartość stałej newMoney
}
// funkcja do usuwania pojedynczych tranzakcji. Ta tranzakcja została wywołana w kodzie html na funkcji onclick
const deleteTransatcion = id => {
    const transactionToDelete = document.getElementById(id);
    console.log(transactionToDelete)
    // tworzymy zmiennką , która pobierze element z id
